...
class matching(object):
    ...
    def __init__(
        self, estimand, k=1, bias_corr_mod=None
    ):
    ...
    def fit(self, y: np.ndarray, w: np.ndarray, X: np.ndarray, **kwargs) -> tuple:
        ...
        if self.estimand == "ATT":
            ...
            bias_corr_term = muhat.predict(X[w == 1, :]).mean() - muhat.predict(
                X[w == 0, :][neighbours.flatten(), :]
            ).mean()
            point_est = y[w == 1].mean() - y0hat.mean() - bias_corr_term
            # variance estimation - Otsu and Rai
            K = np.zeros(n)
            np.put(K, np.argwhere(w == 0), np.bincount(neighbours.flatten(), minlength=n)[w == 0])
            psi = w * (y - muhat.predict(X)) - (1 - w) * (K / self.k) * (y - muhat.predict(X))
